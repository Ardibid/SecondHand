...
def single_letter_test(char_index, mean, std,  model, data, char_data, device, items_to_show):
  ...
  indices = (np.where(np.where(data.Y.detach().cpu().numpy() == 1)[1] == char_index))
  ...
  sample_label = torch.unsqueeze(sample_labels[0], 0).repeat(items_to_show**2, 1)
...
def render_text(text_input, char_data, squeeze_factor= 20, line_padding = 10):
    ...
    image = np.ones(shape=((63+line_padding)*n_lines, (64-squeeze_factor)*line_len))
    ...
    for j, line in enumerate(lines):
        ...
        for i, c in enumerate(line):
            ...
            if (c == " "):
                ...
                try:
                    tmp = char_data[c][0][:-1,squeeze_factor//2:-squeeze_factor//2 ]
                    valid_char = True
                except:
                    pass

            if valid_char:
                image[(63+line_padding)*j:(63+line_padding)*j+63, (64-squeeze_factor)*letter_in_line:(64-squeeze_factor)*(letter_in_line+1)] = tmp
                letter_in_line += 1
